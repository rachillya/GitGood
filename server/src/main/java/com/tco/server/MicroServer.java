...
import com.tco.requests.ShorterTrip.TourRequest;
...
class MicroServer {
    ...
    private String processHttpRequest(spark.Request httpRequest, spark.Response httpResponse, Type requestType) throws BadRequestException {
        setupResponse(httpResponse);
        String jsonString = httpRequest.body();
        try {
            if(jsonString.contains("type") || jsonString.contains("where")){
                log.info("Bad Request - Find request contained parameter for 'type' or 'where");
