...
import ServerSettings,{FeatureRow}from '../../../src/components/Margins/ServerSettings';
...
describe('Server Settings Modal', () => {
    ...
    it('render valid features', async () => {
        user.type(inputBox, validUrl);
        let mockValue =["config", "find"];
        render(<FeatureRow label="Features" value="mockValue"/>);
        await waitFor(() => {
             expect(screen.getByText('config, find')); // should pass
             //expect(screen.getByText('test')); //should fail
        });
    });
});
